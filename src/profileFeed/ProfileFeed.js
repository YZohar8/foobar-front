...
function ProfileFeed() {
  ...
  const refreshPage = async () => {
    ...
    if (!me) {
      const resultStatus = await friendsConnectToDB.getFriendshipStatus(userProfile.id);
  ...
  const handleSearch = async (text) => {
    const result = await getSearchPostsForOneUser(userProfile.id, text);
    if(result.success) {

...
function ShowResponse({ index, response, refresh, userId, setErrorNote, obIsProfile, handleClose}) {
    ...
    const handleDeleteResponse = async () => {
        const result = await commentsConnectToDB.deleteComment(response.postId, response.id, userId);
        if (result.success) {
            refresh();
        } else {
    ...
    return (
        <div key={response.id} className="response-item">
            ...
            <div className="response-user-info">
                ...
                <EditResponse response={response} refresh={refresh} handleClose={handleCloseEditResponse} show={showModalEditResponse} userId={userId} />
            </div>
        </div>
    );
}

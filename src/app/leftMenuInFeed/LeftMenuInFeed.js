...
function LeftMenuFeed({obIsProfile, myUser, updateMyUser,  refreshAllPage, friendsList, setError}) {
    ...
    const refresh = async () => {
        const result = await friendsConnectToDB.getPendingFriends(myUserRef.current.id);
        if (result.success) {
    ...
    const handleWithdeleteUser = async () => {
        ...
        if (confirmLogout) {
            const result = await deleteUser(myUserRef.current.id);
            if (result.success) {
                document.body.classList.remove('dark-mode');
                sessionStorage.clear();
                navigate('/loading', { state: { destination: '/' } });

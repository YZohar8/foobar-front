...
function EditResponse({ response, refresh, handleClose, show, userId }) {
    ...
    const handleEditResponse = async () => {
        ...
        const result = await commentsConnectToDB.updateComment(response.postId, response.id, responseEditText, userId);
        if (result.success) {
            setErrorNote(null);
            handleClose();
            refresh();
